...
inline bool OnUpdateMagnetSouth(avl::CellUpdateContext& ctx)
{
    ctx.notify_chunk();
    ctx.notify_sector();
    ...
    for (int dy = -range; dy <= range; dy += step)
    {
        for (int dx = -range; dx <= range; dx += step)
        {
            ...
            if (avl::utils::get_chance(strength * 0.15f))
            {
                ...
                // Prevent cells from moving onto the magnet itself.
                if (newX == ctx.x && newY == ctx.y)
                    continue;

                if (!ctx.is_out_of_bounds(newX, newY) && ctx.is_empty(newX, newY))
                {
                    uint8_t currentTargetID = ctx.get_cell_id(targetX, targetY);
                    if (currentTargetID == 0 || currentTargetID != targetID)
                        continue;
